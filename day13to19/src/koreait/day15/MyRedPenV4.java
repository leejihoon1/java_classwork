...
public class MyRedPenV4 {
	//45 ~ 55라인 추가 : 틀린문제를 배열에 저장하고 파일 쓰기합니다.(MyFileRw.java 클래스 사용) 
	//전역 변수
	...
	public static void main(String[] args) {
		...
		while(true) {
			int cnt=0;
//			2)
			addList = new ArrayList<>(); // 반복문 시작할 때 마다 새로운  ArrayList 객체를 만들어서 참조합니다.
...
//				maths = addList.toArray(maths);
				...
				for(MathProblem prob : addList) {
					if(prob.isCorrect()==false)
						maths[i++]=prob; //배열에 트린문제만 담깁니다.
				}
				System.out.println(Arrays.toString(maths)); // 변환결과 확인
				// 틀린문제를 파일에 출력하기 :파일명 yourmath.txt 
				MyFileRw.fileWrite("C:\\Temp\\Dev\\yourmath.txt", maths);
				
				
				for( MathProblem prob : addList ) {
					if(!prob.isCorrect()) // (prob.isCorrect()==false)
	...
	//메소드를 만들어서 수정합니다. 메소드는 코드 재사용성과 가독성을 위해서 필요합니다.
	static int start(int num, char op) { // 리턴 Int 맞은 개수   인자 int num 문제갯수   // char op  연산자 +,-,*,/
		int cnt =0;
		int ans;
		
		for(int i=0;i<num;i++) { // 힌트 : 100/문제개수 값이 문제 1개당 점수 입니다.
			MathProblem p1 = new MathProblem(op);
			addList.add(p1); // 새로운문제 리스트에 담습니다.
			...
			//sc.nextInt(); 메소드 실행의 엔터는다음 sc.nextInt()메소드에서 가져갑니다. -> 오류발생의 원인
