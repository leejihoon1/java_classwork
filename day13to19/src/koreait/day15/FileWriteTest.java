...
//	파일 출력의 기본 : 경로(어디에 있는 파일인지), 파일명 (파일이름)
	public static void main(String[] args) {
		...
		String filename = "C:\\Temp\\Dev\\자바테스트.txt"; //역슬래쉬 \는  \\로 해야합니다.
		
		// 파일객체를 생성합니다.
		File file = new File(filename);
		
		//file 참조하는 파일 객체를 데이터 출력에 사용하기 위한 클래스는 PrintWriter입니다.
		try { // 파일입력 또는 파일출력 할 때 자바는 꼭 try-catch로 처리하도록 합니다.
