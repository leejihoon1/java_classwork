...
public class MyDictionaryApp { // 애플리케이션 클래스 ( main 메소드를 응용프로그램 실행을 시작합니다.)
	...
	public static void main(String[] args) {
		...
		while(run) {
			...
			String filename; // = "C:\\Temp\\Dev\\mydict.txt";
	...
	//예시 : myword에 저장된 내용이 milk : 우유, book : 책, car : 자동차 라고 할때
	//	       지금 입력된 eng는 book, kor은 예약하다 라고 한다면 기존에 저장된 단어일 떄 koreans 필드에 새로운 뜻의 kor만 요소로 추가한다.
	...
	private static void wordAdd() { // 단어 추가
		...
		do {
			...
			// 입력받은 eng가 mywords 리스트에 있느냐? 있다면 -몇번째 인덱스인지 알아내서 그 위치에 kor담기, 
...
//					index = i; // index 변수는 for문 밖에서  같은 단어의 인덱스 위치 값을 활용할 떄 사용히면 됩니다.
					flag  = true;
					koreans = w.getKoreans(); // 이미 존재하는 리스트 객체를 가져와서 korean변수가 참조합니다.
					koreans.add(kor);
					break;
				}
			}
			
			//index 변수가 -1 이라면 단어장에 없는단어 -kor 추가할 리스트 생성해서 담기 ->Word 객체 생성해서 myword에 추가하기	
			if(!flag) { // if(index == -1;){
	...
	// 파일에 단어들을 저장합니다. (저장할 파일 경로와 파일 이름을  filename으로 전달
	private static void wordSave(String filename) { 
		...
		try {
			...
			System.out.println("없는 경로 또는 파일 입니다.");
		}
	}
	...
	private static void wordRead(String filename) { // 파일에 저장된 단어들 읽어옵니다. 
		...
		try {			
			...
			while(fsc.hasNext()) { //파일 읽기 -> mywords 리스트에 저장된 객체로 생성해야 합니다.
//				System.out.println(fsc.nextLine()); //  테스트 1
				StringTokenizer stk = new StringTokenizer(fsc.nextLine(),":[]");
									// 구분기호 문자열은 ':' 또는  '[' 또는 ']' 문자를  구분자로 사용한다.
				while(stk.hasMoreTokens()) { // 파일에서 1라인씩 읽어오고 그것을 토큰화 하기.->Word 객체 생성할 필드 값
//					System.out.println(stk.nextToken(":[")); // 테스트2
					Word w = new Word(stk.nextToken()); // 첫번쨰 english 필드값 저장
					List<String> kor = Arrays.asList(stk.nextToken().split(", ")); 
					// split결과 타입은 배열을 리스트로 변환
					
					kor = new ArrayList<String>(kor); // 고정리스트로 변경할 수 있는 리스트 객체로 생성하기 
					w.setKoreans(kor); // 두번째 토큰을 리스트로 변환해서 koreans 필드 값으로 참조.
					mywords.add(w);    // Word 객체를 mywords 리스트에 추가하기
				} // substring 메소드로 "]" 제외 : String temp = stk.nextToken().substring(0,temp-1).split(", ");
			}
			fsc.close();
		} catch (FileNotFoundException e) {
			System.out.println("없는 경로 또는 파일 입니다.");
