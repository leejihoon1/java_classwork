...
public class ObjectArgMethod { // Object가 인자인 메소드
	// int 배열의 정수 값 중에서 최대값 / 최소값 구하는 메소드를 만듭니다. // 참조값을 인자로 전달하는 연습합니다.
	public static void main(String[] args) {
		...
		int max; // max 초기값 : 1)배열 인덱스 0번값 2) 비교될 다른 값들보다 작은값
		...
		int min; // min 초기값 : 1) 배열 인덱스 0번 값 2)비교될 다른 값들보다 큰 값
					// 비교 범위 : 1) 배열인덱스 1 ~ 마지막인덱스 2)0 ~ 마지막인덱스
		min = 999; // 100으로 해도 됩니다. 2)로 구현
		for (int i = 0; i < arr.length; i++) {
			if (arr[i] < min) // 최소값은 다른 값들보다 작은 값이 되어야 하므로, 그렇지 않을 경우
				min = arr[i]; // min의 값을 그 값으로 변경합니다.
		}
...
//			darr[i] = r.nextDouble() * 9 + 1; // 1 ~ 9 사이의 실수
		}
...
//				대표적인 메소드 - Arrays.toString()메소드 입니다.
	...
	// 메소드 작성으로 구현해봅니다. : 최대값 / 최소값을 구하기 위해 다룰 데이터는 배열에 있습니다. -> 배열 변수는 참조변수.
