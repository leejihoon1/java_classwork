...
//		String클래스의 메소드실행 : charAt(0), subStrinh(2,5)를 각 배열 요소 문자열로 실행합니다.
		for(int i =0; i<strArray.length;i++) {
		...
		for(int i =0 ; i <names.length;i++) {
			...
			//String은 클래스, 문자열은 객체 -> names 배열의 주소, names[i]는 문자열 주소 둘다 참조 (주소)값을 저장합니다. 
			 
//		System.out.println("charAt(0) : " + names[0].charAt(0) + 
//				", subString(2,5) : " + names[0].substring(2,5)); // 문자열이 null일때 charAt 등 메소드 실행 오류 발생.
//											-> 오류의 원인은 NullPointerException
		}	
		...
		//names 배열의 영문자를 모두 대문자로 변화하여 names 배열 요소로 참조(지정)하도록 합니다.
			for(int i =0; i<names.length;i++) {
				names[i] = names[i].toUpperCase(); // names배열 요소들이 참조하던 주소가 변경됩니다.
